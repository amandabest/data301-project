...
temp <- temp %>%
  ...
  mutate(month_year = (month_year)) #this makes it break
...
server <- function(input, output, session) {
  ...
  output$timePlot <- renderPlotly({
    ...
    p <- ggplot(data, aes(x = date, y = temperature, group = site)) +
      geom_line(size=0.1) +
      labs(title = "Temperatures Over Time for Each Site",
