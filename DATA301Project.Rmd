...
server <- function(input, output, session) {
  ...
  output$ts_year_range_ui <- renderUI({
    if (input$ts_year_type == "Year Range") {
      sliderInput("ts_year_range", "Select Year Range:", sep="", min = min(temp$year), max = max(temp$year),
                  value = c(1990, max(temp$year)), step = 1)
    } else {
      NULL
    }
  })
  ...
  output$timePlot <- renderPlotly({
    ...
    p <- ggplot(data, aes(x = date, y = temperature, color = site)) +
      geom_line() +
      labs(title = "Daily Temperatures Over Time for Each Site",
...
This is where we'll outline who did what
